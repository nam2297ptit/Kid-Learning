...
//img
import { CustomImg } from "./CustomTag";
            ...
            if (!ValidInput.isEmpty(data.diff_values.attachments.new)) {
                buffer.push(
                    <div key={utils.randomString()}>
                        <Badge
                            color='primary'
                            className='pt-1 aria-label '
                            style={{ fontSize: 13 }}>
                            new attachments
                        </Badge>
                        <span className='Timeline___linkto pl-2'>
                            {data.diff_values.attachments.new.subject}
                        </span>
                    </div>,
                );
            }
            ...
            if (!ValidInput.isEmpty(data.diff_values.attachments.deleted)) {
                buffer.push(
                    <div key={utils.randomString()}>
                        ...
                        <span className='Timeline___linkto pl-2'>
                            {data.diff_values.attachments.deleted.subject}
                        </span>
                    </div>,
                );
            }
        ...
        if (data.diff_values.description !== undefined) {
            buffer.push(
                <div key={utils.randomString()}>
                    ...
                    <span className='Timeline___linkto pl-2'>
                        {data.diff_values.description[0] + " => " + data.diff_values.description[1]}
                    </span>
                </div>,
            );
        }
        ...
        if (data.diff_values.assigned_users !== undefined) {
            buffer.push(
                <div key={utils.randomString()}>
                    ...
                    <span className='Timeline___linkto pl-2'>
                        {data.diff_values.assigned_users[0] +
                            " => " +
                            data.diff_values.assigned_users[1]}
                    </span>
                </div>,
            );
        }
        if (data.diff_values.status !== undefined) {
            buffer.push(
                <div key={utils.randomString()}>
                    ...
                    <span className='Timeline___linkto pl-2'>
                        {data.diff_values.status[0] + " => " + data.diff_values.status[1]}
                    </span>
                </div>,
            );
        }
        if (data.diff_values.tags !== undefined) {
            buffer.push(
                <div key={utils.randomString()}>
                    ...
                    <span className='Timeline___linkto pl-2'>
                        {data.diff_values.tags[0] + " => " + data.diff_values.tags[1]}
                    </span>
                </div>,
            );
        }
        if (data.diff_values.subject !== undefined) {
            buffer.push(
                <div key={utils.randomString()}>
                    ...
                    <span className='Timeline___linkto pl-2'>
                        {data.diff_values.subject[0] + " => " + data.diff_values.subject[1]}
                    </span>
                </div>,
            );
        }
        ...
        if (data.diff_values.type !== undefined) {
            buffer.push(
                <div key={utils.randomString()}>
                    ...
                    <span className='Timeline___linkto pl-2'>
                        {data.diff_values.type[0] + " => " + data.diff_values.type[1]}
                    </span>
                </div>,
            );
        }
        if (data.diff_values.severity !== undefined) {
            buffer.push(
                <div key={utils.randomString()}>
                    ...
                    <span className='Timeline___linkto pl-2'>
                        {data.diff_values.severity[0] + " => " + data.diff_values.severity[1]}
                    </span>
                </div>,
            );
        }
        if (data.diff_values.priority !== undefined) {
            buffer.push(
                <div key={utils.randomString()}>
                    ...
                    <span className='Timeline___linkto pl-2'>
                        {data.diff_values.priority[0] + " => " + data.diff_values.priority[1]}
                    </span>
                </div>,
            );
        }
                        ...
                        <Media body className='p-1 ml-3'>
                            <strong>
                                <Link
                                    ...
                                    className='Activities___linkto text-success'>
                                    {data.user.full_name}
                                </Link>{" "}
                            </strong>
        ...
        return (
            <React.Fragment>
                {!this.state.isLoaderAPI ? (
                    ...
                    <ContentActivities dataActivities={this.state.dataActivities} />
                )}
            </React.Fragment>
        );
    }
