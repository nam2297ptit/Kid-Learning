...
class Wiki extends React.Component {
  ...
  getTimeline() {
    this.setState({ isLoadall: true, isLoad: true, isLoadListWiki: true, isLoadHis: true, isLoadFile: false, });
    api.getTimeline((err, result) => {
      if (err) {
        ...
        this.setState({
          isLoadall: false, listWiki: result, isLoad: false, isLoadListWiki: false, isLoadHis: false, isLoadFile: false, firstWiki: result[0]
        })
    ...
    if (ValidInput.isEmpty(e)) {
      ...
      api.CreateWiki(e, (err, result) => {
        if (err) {
          ...
          this.setState({ isLoadListWiki: false, listWiki: listWiki, firstWiki: result, check: true });
  ...
  // Xoa Wiki
  DeleteWiki(e, item) {
    if (item.owner.id === this.props.user.id || JSON.parse(sessionStorage.getItem('project')).i_am_admin) {
      this.setState({ isLoadListWiki: true });
    ...
    if (ValidInput.isEmpty(this.state.nameWikiNo1)) {
      ...
      api.CreateWiki(this.state.nameWikiNo1, (err, result) => {
        if (err) {
          ...
          var listWiki = utils.copyState(this.state.listWiki)
          listWiki.push(result)
          this.setState({ isOpen: !this.state.isOpen, isLoadListWiki: false, listWiki: listWiki, firstWiki: result, check: true });
          this.getWiki(result.id)
        }
      })
    }
  ...
  render() {
    ...
    return (
      ...
      <React.Fragment>
        {this.state.isLoadall ? <LoadingSprinner /> :
          ...
          <Container fluid className="p-0">
            <Modal
              ...
              centered>
              <ModalHeader >
                Chưa có wiki nào, mời bạn tạo 1 wiki:
       </ModalHeader>
              <ModalBody>
