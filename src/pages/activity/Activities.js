...
import { Button, Card, CardBody, CardHeader, CardTitle, Media } from "reactstrap";
...
class Activities extends React.Component {
    ...
    renderDataEventType(data) {
        switch (data.event_type) {
            ...
            case "projects_memberships.change":
                return <React.Fragment>membership change role?</React.Fragment>;
                ...
                return (
                    <React.Fragment>
                        ...
                        {data.data.user.id.diff.subject === undefined ? null : (
                            <React.Fragment>
                                "Subject" of the US from {" " + data.data.user.id.diff.subject[0]}{" "}
                                to {data.data.user.id.diff.subject[1]}
                            </React.Fragment>
                        )}
                        ...
                        {data.data.values_diff.status === undefined ? null : (
                            <React.Fragment>
                                ...
                                from {" " + data.data.values_diff.status[0]} to{" "}
                                {data.data.values_diff.status[1]}
                            </React.Fragment>
                        )}
                        {data.data.values_diff.due_date === undefined ? null : (
                            <React.Fragment>
                                ...
                                <Link
                                    ...
                                    className='Timeline___linkto'>
                                    {"#" +
                                        data.data.userstory.ref +
                                        " " +
                                        data.data.userstory.subject}
                                </Link>{" "}
                                from {" " + data.data.values_diff.due_date[0]} to{" "}
                                {data.data.values_diff.due_date[1]}
                            </React.Fragment>
                        )}
                    </React.Fragment>
                );
            case "tasks.task.create":
                return (
                    <React.Fragment>
                        ...
                        <Link to={"#"} activeclassname='active' className='Timeline___linkto'>
                            {"#" +
                                data.data.task.userstory.ref +
                                " " +
                                data.data.task.userstory.subject}
                        </Link>
                    </React.Fragment>
                );
                    ...
                    <React.Fragment>
                        ...
                        {data.data.values_diff.subject === undefined ? null : (
                            <React.Fragment>
                                ...
                                from {" " + data.data.values_diff.subject[0]} to{" "}
                                {data.data.values_diff.subject[1]}
                            </React.Fragment>
                        )}
                    </React.Fragment>
                );
            case "projects_memberships.create":
                return <React.Fragment>is a new member </React.Fragment>;
            default:
                return null;
        }
    }
    ...
    render() {
        ...
        let actions = this.state.dataTimeline.slice(0, this.state.max).map((data, index) => {
            const linkUser = `/profile${
                data.data.user.id === utils.getUserId() ? "" : `?email=${data.data.user.username}`
            }`;
            return (
                <div key={index}>
                    <Media>
                        ...
                        <Media body className='p-2'>
                            <React.Fragment>
                                ...
                                <Link to={linkUser} className='Timeline___linkto'>
                                    <strong>{data.data.user.id.user_id}</strong>
                                </Link>{" "}
        ...
        return (
            <Card>
                ...
                <CardBody>
                    {actions}
                    {this.state.max < this.state.dataTimeline.length ? (
                        <Button
                            block
                            color='primary'
                            className='load-more'
                            onClick={() => this.setState({ max: this.state.max + 10 })}>
                            Load More
                        </Button>
